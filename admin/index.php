...
?>
    ...
    unset($_SESSION['success_message']); // Clear the message after displaying it
            ...
            if (isset($_GET['messages'])) {
                echo '<div class="alert alert-success">' . $_GET['messages'] . '</div>';
            }
?></a>
                </div>
                <div class="col-md-14">
          ...
          <div class="col-md-6">
            <div class="card ">
              ...
              <div class="card-body">
                <div class="table-responsive ps">
                  <table class="table table-hover tablesorter " id="">
                    <thead class=" text-primary">
                        <tr><th>ID</th><th>Vendor Name</th><th>Company Name</th>
                    </tr></thead>
                    <tbody>
                      <?php 
                        $result=mysqli_query($con,"select * from vendor")or die ("query 1 incorrect.....");
                        $i=1;
                        while(list($vendor_id,$vendor_name,$vendor_cn)=mysqli_fetch_array($result))
                        {	
                            ...
                            $count=$row["count_items"];
                            $i++;*/
                        echo "<tr><td>$vendor_id</td><td>$vendor_name</td><td>$vendor_cn</td>
            ...
            <div class="card ">
              <div class="card-header card-header-primary">
                <h4 class="card-title">Designers List</h4>
              </div>
              <div class="card-body">
                <div class="table-responsive ps">
                  <table class="table table-hover tablesorter " id="">
                    <thead class=" text-primary">
                        <tr><th>ID</th><th>Designers Name</th><th>Email</th>
                    </tr></thead>
                    <tbody>
                      <?php 
                        $result=mysqli_query($con,"select * from designers")or die ("query 1 incorrect.....");
                        $i=1;
                        while(list($ds_id,$ds_name,$ds_email)=mysqli_fetch_array($result))
                        {	
                            /*$sql = "SELECT COUNT(*) AS count_items FROM products WHERE product_cat=$i";
                            $query = mysqli_query($con,$sql);
                            $row = mysqli_fetch_array($query);
                            $count=$row["count_items"];
                            $i++;*/
                        echo "<tr><td>$ds_id</td><td>$ds_name</td><td>$ds_email</td>
